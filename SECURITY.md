...
| Input validation | `jakarta.validation` on every controller DTO | Implemented in `product-service`, `inventory-service`, `crm-service`; not yet in the other services | Per-service DTOs |
| SQL injection | Parameterized queries only (JPA/Hibernate, no string-concatenated SQL) | Implemented in `product-service`, `inventory-service`, `crm-service` (Spring Data JPA/JPQL only, no native queries) | Per-service repositories |
